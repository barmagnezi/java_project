...
public class AstarSearcherTest {
	...
	public void test() {
		...
		try{
			for (int i = 0; i < 3; i++) {
				AstarSearcher AstarSearcher=new AstarSearcher(new MazeAirDistance());
				...
				while(sol.size()!=1){
					...
					if(!CheckMotion(Integer.parseInt(Current[1]),Integer.parseInt(Current[0]),Integer.parseInt(next[1]),Integer.parseInt(next[0]),mm,true))
	...
	public boolean CheckMotion(int CurrentCol,int CurrentRow,int nextCol,int nextRow,Maze maze,boolean Diagonals){
		boolean flag=true;
		if(nextCol<0 || nextRow<0 || nextCol==maze.getCols() || nextRow==maze.getRows())
			return false;
		if(!(nextCol!=CurrentCol &&CurrentRow!=nextRow)){
			if(CurrentCol+1==nextCol)
				flag=flag&&(!maze.getCell(CurrentRow, CurrentCol).getRightWall().isExist());
			if(CurrentCol-1==nextCol)
				flag=flag&&(!maze.getCell(nextRow, nextCol).getRightWall().isExist());
			if(CurrentRow+1==nextRow)
				flag=flag&&(!maze.getCell(CurrentRow, CurrentCol).getBottomWall().isExist());
			if(CurrentRow-1==nextRow)
				flag=flag&&(!maze.getCell(nextRow, nextCol).getBottomWall().isExist());
			return flag;
		}else{
			if (Diagonals==true){
				int col=CurrentCol;
