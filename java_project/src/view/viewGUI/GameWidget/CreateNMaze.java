...
public class CreateNMaze extends BasicWindow {
	...
	protected void initWidgets() {
		...
		LBEMazeName.setLayoutData(new GridData(SWT.LEFT, SWT.TOP, false, false,
				1, 1));

		Text TMazeName = new Text(shell, SWT.BORDER);
		TMazeName.setLayoutData(new GridData(SWT.None, SWT.None, false, false,
				3, 1));
		...
		BOpenMaze.setLayoutData(new GridData(SWT.RIGHT, SWT.None, false, false,
				4, 1));
		...
		BOpenMaze.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent arg0) {
				...
				if (TMazeName.getText().length() == 0) { // Name check(!=null)
					...
					messageBox
							.setMessage("The name of the maze can't be empty");
				...
				if (flag == false && !isInteger(Trow.getText())) {
					MessageBox messageBox = new MessageBox(shell, SWT.OK);
					messageBox
							.setMessage("The number of rows have to be an integer");
					messageBox.setText("Error");
					messageBox.open();
					Trow.setFocus();
					flag = true;
					...
					if (flag == false && Integer.parseInt(Trow.getText()) < 3) {
						MessageBox messageBox = new MessageBox(shell, SWT.OK);
						messageBox
								.setMessage("The number of rows have to be >2");
						messageBox.setText("Error");
						messageBox.open();
						Trow.setFocus();
						flag = true;
				...
				if (flag == false && Tcol.getText().length() == 0) {
					MessageBox messageBox = new MessageBox(shell, SWT.OK);
					messageBox
							.setMessage("The number of columns can't be empty");
					messageBox.setText("Error");
					messageBox.open();
					Tcol.setFocus();
					flag = true;
				...
				if (flag == false && !isInteger(Tcol.getText())) {
					MessageBox messageBox = new MessageBox(shell, SWT.OK);
					messageBox
							.setMessage("The number of columns have to be an integer");
					messageBox.setText("Error");
					messageBox.open();
					Tcol.setFocus();
					flag = true;
					...
					if (flag == false && Integer.parseInt(Tcol.getText()) < 3) {
						MessageBox messageBox = new MessageBox(shell, SWT.OK);
						messageBox
								.setMessage("The number of columns have to be >2");
						messageBox.setText("Error");
						messageBox.open();
						Tcol.setFocus();
						flag = true;
				...
				if (flag == false) {
					mazeView.generateMaze(TMazeName.getText(),
							Integer.parseInt(Trow.getText()),
							Integer.parseInt(Tcol.getText()));
					shell.dispose();
				}
			}
